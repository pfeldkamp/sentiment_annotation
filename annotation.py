...
#                 if elapsed_time > 120 and len(st.session_state.tmp) >= 5:
#                     #for r in st.session_state.tmp.values.tolist():
#                     sheet.append_rows(st.session_state.tmp.values.tolist())
...
# CONFIG
ROOT_DIR = "data"
DATA_FILE = f"{ROOT_DIR}/sentences.csv"
ANNOT_FILE = f"{ROOT_DIR}/annotations.csv"
SHEET_NAME = "feuilleton_annotations"

# -------------------------------
# HELPER: SAFE APPEND WITH RETRY
def safe_append_rows(sheet, rows, max_attempts=5):
    for attempt in range(max_attempts):
        try:
            ...
            if "429" in str(e):
                ...
                return False
    st.error("Failed to write to Google Sheets after retries.")
    return False

# -------------------------------
# CREATE LOCAL ANNOTATIONS FILE
if not os.path.exists(ANNOT_FILE):
    pd.DataFrame(columns=["annotator", "text_id", "text", "sentiment_score"]).to_csv(ANNOT_FILE, index=False)
...
if st.session_state.username.strip():
    st.sidebar.markdown(
        "<div style='background-color:#d4edda; padding:5px; border-radius:5px;'>✅ Initialer indtastet</div>",
        unsafe_allow_html=True
    )
...
if st.sidebar.button("Gem alle annoteringer nu"):
    if not st.session_state.tmp.empty:
        safe_append_rows(sheet, st.session_state.tmp.values.tolist())
        st.session_state.tmp = pd.DataFrame()
        st.session_state.start_time = time.time()
        st.sidebar.success("Alle annoteringer gemt ✅")
    else:
        st.sidebar.info("Ingen annoteringer at gemme.")
    ...
    if st.session_state.idx >= len(df):
        ...
        if st.button("Gem"):
            if st.session_state.username.strip() == "":
                ...
                # Time/row-based flush to Google Sheets
                elapsed = time.time() - st.session_state.start_time
                if elapsed > 120 and len(st.session_state.tmp) >= 5:
                    safe_append_rows(sheet, st.session_state.tmp.values.tolist())
                    st.session_state.tmp = pd.DataFrame()
                    st.session_state.start_time = time.time()
